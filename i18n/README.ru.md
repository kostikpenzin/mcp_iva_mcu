...
- **Общаться в чате** — создавать групповые чаты, отправлять и редактировать сообщения, пересылать, отмечать, искать, управлять уведомлениями
...
- **Опросы и анкеты** — создавать опросы, собирать ответы, экспортировать результаты
...
### 6. Создать опрос во время конференции

> **Вы говорите:** "Создай опрос: 'Выберите время для следующей встречи' с вариантами 'Пн', 'Вт', 'Ср'"

AI-агент:
1. Вызовет `iva_conference_inquiry` с `action: "create"` и данными опроса
2. Вызовет `iva_conference_inquiry` с `action: "start"` для запуска опроса

### 7. Создать пользователя (Integration API)
...
### 8. Получить статистику по конференции
